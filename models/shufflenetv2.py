...
class ShuffleNetV2(nn.Module):
    def __init__(self, nIn, nOut=6, width_mult=1.):
        ...
        self.conv1 = conv_bn(nIn, input_channel, stride=(1, 2, 2))
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(0.2),
            nn.Linear(self.stage_out_channels[-1], nOut)
        )
...
if __name__ == "__main__":
    model = get_model(nIn=1, nOut=6, width_mult=1.)
    #model = model.cuda()
    model = nn.DataParallel(model, device_ids=None)
    print(model)

    #input_var = Variable(torch.randn(100, 1, 3, 3, 272))
    input_var = Variable(torch.randn(128, 3, 3, 272))
