...
FMC_Str = 'R_FMC_DNN'
FMC_LDA_Str = 'R_FMC_LDA_RF'


config.set_mode('five_reg_mode')
pd.set_option('display.max_columns', 10)

parser = argparse.ArgumentParser(description='2D CNN for hyperspectral image classification')
parser.add_argument('-me', '--method', help='method, R_2D_CNN, R_3D_CNN, R_ShuffleNetV2, R_Conv3d_LSTM, R_Conv3d_GRU, R_RNN, R_JointDeepModel, R_FMC, MixModel, SqueezeNet, Resnet, Resnext'
                                            'R_SVM, R_RandomForest, R_GradientBoosting, R_LogisticRegression, R_SVM-RBF, R_LDA, FMC_LDA_Str', default='R_LDA')
...
parser.add_argument('--iter_num',dest='iter_num',default=500)
...
def pred_to_integer(pred):
    pred_int = np.zeros_like(pred)
    for i in range(pred.shape[0]):
        if pred[i] < 0:
            ...
            pred_int[i] = round(pred[i])
    return pred_int


def integer_to_pro(pred):
    pred_pro = []
    for i in range(pred.shape[0]):
        if pred[i] <= 0:
            pred_pro.append([1, 0, 0, 0, 0])
        elif pred[i] <= 1:
            pred_pro.append([1-pred[i], pred[i], 0, 0, 0])
        elif pred[i] <= 2:
            pred_pro.append([0, 2-pred[i], pred[i]-1, 0, 0])
        elif pred[i] <= 3:
            pred_pro.append([0, 0, 3-pred[i], pred[i]-2, 0])
        elif pred[i] <= 4:
            pred_pro.append([0, 0, 0, 4-pred[i], pred[i]-3])
        elif pred[i] > 4:
            pred_pro.append([0, 0, 0, 0, 1])
    return np.array(pred_pro)
...
class RNNMethod:
    def __init__(self, method, args):
        ...
        if method == 'R_2D_CNN':
            self.model = Conv2dModel(self.cropped_dim, (args.cube_size, args.cube_size), self.class_num).to(device)
        elif method == 'R_3D_CNN':
            self.model = Conv3dModel(1, self.cropped_dim, (args.cube_size, args.cube_size, 3), self.class_num).to(device)
        elif method == 'R_Conv3d_LSTM':
            self.model = Conv3d_RNN(1, (args.cube_size, args.cube_size, 3), self.class_num, 'lstm').to(device)
        elif method == 'R_Conv3d_GRU':
            self.model = Conv3d_RNN(1, (args.cube_size, args.cube_size, 3), self.class_num, 'gru').to(device)
        elif method == 'R_RNN':
            self.model = MLayerBiLSTMModel(1, self.class_num).to(device)
        elif method == 'R_ShuffleNetV2':
            self.model = ShuffleNetV2(nIn=1, nOut=self.class_num).to(device)
        elif method == 'R_JointDeepModel':
            self.model = JointDeepModel(nIn_2d=self.cropped_dim, nIn_3d=1,
                                        nkernel_2d=(args.cube_size, args.cube_size), nkernel_3d=(args.cube_size, args.cube_size, 3),
                                        nOut=self.class_num, rnn_type='gru').to(device)
        elif method == FMC_Str:
            self.model = FMC(nIn_2d=self.cropped_dim, nOut=self.class_num).to(device)
        elif method == 'R_SqueezeNet':
            self.model = SqueezeNet(ndim=self.cropped_dim, num_classes=self.class_num).to(device)
        elif method == 'R_ResNet34_3D':
    ...
    def train(self, loader, epoch, premodels=None):
        ...
        if epoch % 100 == 0:
            ...
            batch_y = batch_y.cpu()
            pred_int = pred_to_integer(pred.cpu().detach().numpy())
            acc = 100 * sum(pred_int == batch_y.numpy()) / batch_y.shape[0]
            macro_f1 = f1_score(batch_y, pred_int, average='macro')
    ...
    def val(self, loader, premodels=None):
        ...
        for step, (batch_x, batch_y) in enumerate(loader):
            ...
            preds = np.concatenate((preds, pred[:, 0].to('cpu').detach().numpy()))

        loss = self.loss_fn(torch.tensor(preds), torch.tensor(labels)).item()
        preds_int = pred_to_integer(preds)
        acc = 100 * sum(preds_int == labels) / labels.shape[0]
        macro_f1 = f1_score(labels, preds_int, average='macro')
    ...
    def metrics(self, preds, labels):
        pred_tmp = preds.copy()
        loss = self.loss_fn(torch.tensor(preds), torch.tensor(labels)).item()
        mse = nn.MSELoss()(torch.tensor(preds.astype(np.float32)), torch.tensor(labels.astype(np.float32))).item()
        preds_int = pred_to_integer(preds)
        acc = 100 * sum(preds_int == labels) / labels.shape[0]
        macro_f1 = f1_score(labels, preds_int, average='macro')
        print(f"Test accuracy: {acc :>0.2f}%, loss: {loss:>8f}, macro-f1: {macro_f1:>8f}")
        matrix = confusion_matrix(labels, preds_int)
        ...
        # kappa
        kappa_value = cohen_kappa_score(labels, preds_int)
        #mcc
        mcc = matthews_corrcoef(labels, preds_int)
        kk = 0
        ...
        return oa, aa, kappa, ac_list, loss, macro_f1, pred_tmp, labels, matrix_pro, mse, mcc

    def metrics_pro(self, preds, labels):
        pred_tmp = preds.copy()
        preds = preds.argmax(1)
        loss = nn.SmoothL1Loss()(torch.tensor(preds.astype(np.float32)), torch.tensor(labels.astype(np.float32))).item()
        mse = nn.MSELoss()(torch.tensor(preds.astype(np.float32)), torch.tensor(labels.astype(np.float32))).item()
    ...
    def voting_test(self, loader, models_weights, voting_models=None):
        ...
        with torch.no_grad():
            ...
            for step, (batch_x, batch_y) in enumerate(loader):
                ...
                for index in range(len(voting_models)):
                    ...
                    models_pred.append(integer_to_pro(pred[:, 0].cpu().detach().numpy()) * models_weights[index])
                models_pred = np.sum(np.array(models_pred), axis=0)
        ...
        return self.metrics_pro(models_preds, labels)


    def get_features(self, dataset):
        self.model.eval()
        _, features = self.model(dataset['data'])
...
def train_tradition_algs(algorithm, args, loader, best_model, premodels=None):
    ...
    if algorithm == 'R_SVM-RBF':
        model = svm.SVC(kernel='rbf')
    elif algorithm == 'LogisticRegression':
        model = LogisticRegression(random_state=0, multi_class='auto')
    elif algorithm == 'R_RandomForest':
        model = RandomForestRegressor(n_estimators=100)
        #model = RandomForestClassifier(n_estimators=200)
    elif algorithm == 'R_GradientBoosting':
        model = GradientBoostingRegressor(n_estimators=100)
    elif algorithm == 'R_LDA':
...
def test_tradition_algs(algorithm, args, test_data, best_model, loader, premodels=None):
    ...
    loss = nn.SmoothL1Loss()(torch.tensor(preds.astype(np.float32)), torch.tensor(y_test.astype(np.float32))).item()
    mse = nn.MSELoss()(torch.tensor(preds.astype(np.float32)), torch.tensor(y_test.astype(np.float32))).item()
    preds_int = pred_to_integer(preds)
    acc = 100 * sum(preds_int == y_test) / y_test.shape[0]
    macro_f1 = f1_score(y_test, preds_int, average='macro')
    print(f"Test accuracy: {acc :>0.2f}%, loss: {loss:>8f}, macro-f1: {macro_f1:>8f}")
    ...
    print(f'oa: {accuracy:0.4f}')
    # kappa
    kappa_value = cohen_kappa_score(y_test, preds_int)
    # mcc
    mcc = cohen_kappa_score(y_test, preds_int)
    kk = 0
    for i in range(matrix.shape[0]):
        kk += np.sum(matrix[i]) * np.sum(matrix[i, :])
    pe = kk / (np.sum(matrix) * np.sum(matrix))
    ...
    print(f'kappa: {kappa:0.4f}')
    matrix_pro = np.round(matrix / np.sum(matrix, axis=1), 4)
    return oa, aa, kappa, ac_list, loss, macro_f1, preds, matrix_pro, matrix, y_test, mse, mcc

def is_tradition_alg(algorithm):
    if (algorithm == 'R_SVM-RBF') or \
            (algorithm == 'R_LogisticRegression') or \
            (algorithm == 'R_RandomForest') or \
            (algorithm == 'R_GradientBoosting') or \
            (algorithm == 'R_LDA') or \
            (algorithm == FMC_LDA_Str) or \
            (algorithm == 'R_MixModel'):
        return True
    return False
...
def multi_process_kfold(algorithm, data_type='SWIR'):
    ...
    for data_file in data_file_list:
        ...
        for loop in range(1):
            ...
            pre_models = []
            model_i = RNNMethod('R_2D_CNN', args_copy)
            if (args.method == FMC_Str) or (args.method == FMC_LDA_Str):
                config.loss_min = 0.00001
                #pretrained_models = ['R_2D_CNN', 'R_RNN', 'R_3D_CNN']
                pretrained_models = ['R_ResNet34_2D', 'R_RNN', 'R_ResNet34_3D']
            ...
            start_time_single_fold = datetime.now()
            if is_tradition_alg(algorithm):
                oa, aa, kappa, ac_list, loss, f1, pred, matrix_pro, matrix, labels, mse, mcc = test_tradition_algs(algorithm, args_copy, test_data, best_model, test_data_loader, pre_models)
            else:
                method = RNNMethod(algorithm, args_copy)
                method.model.load_state_dict(torch.load(best_model, map_location='cpu'))
                oa, aa, kappa, ac_list, loss, f1, pred, labels, matrix_pro, mse, mcc = method.test(test_data_loader, pre_models)
            end_time_single_fold = datetime.now()
            total_time_single_fold = (end_time_single_fold-start_time_single_fold).total_seconds()
            total_time_kfold += total_time_single_fold
            test_score_list.append([algorithm, data_file, loop, oa, aa, kappa, ac_list, loss, f1, mse, mcc, total_time_single_fold / len(labels)])
            matrix_pro_list.append(matrix_pro)
            pred_list.append(integer_to_pro(pred))
        pred_arr = np.array(pred_list)
        pred_arr = np.mean(pred_arr, axis=0)
        oa, aa, kappa, ac_list, loss, f1, pred, labels, matrix_pro, mse, mcc = model_i.metrics_pro(pred_arr, labels)
        voting_test_score_list.append(['voting_models_fmc_lda_r', data_file, oa, aa, kappa, ac_list, loss, f1, mse, mcc, total_time_kfold / len(labels)])
        voting_matrix_pro_list.append(matrix_pro)

    print('test_score_list:')
    df = pd.DataFrame(data=test_score_list, columns=['algorithm', 'data_file', 'loop', 'oa', 'aa', 'kappa', 'acc_list', 'loss', 'f1', 'mse', 'mcc', 'time'])
    print(df)
    print('ave_time HSI-A: {}, HSI-B: {}, HSI-C: {}'.format(np.mean(df['time'][:10]), np.mean(df['time'][10:20]), np.mean(df['time'][20:])))
    ...
    df = pd.DataFrame(data=voting_test_score_list,
                      columns=['algorithm', 'data_file', 'oa', 'aa', 'kappa', 'acc_list', 'loss', 'f1', 'mse', 'mcc', 'time'])
    print(df)
    df.to_csv(os.path.join(args.result, data_type + '_' + str(file_no) + '_' + algorithm + '_softvoting_test_scores_voting_models.csv'), index=False)
    pickle.dump(voting_matrix_pro_list, open(
        os.path.join(args.result, data_type + '_' + str(file_no) + '_' + algorithm + '_softvoting_matrix_probabilities_voting_models.pkl'), 'wb'))

def multi_process_kfold_softvoting(data_type='SWIR'):
    models_names = ['R_2D_CNN', 'R_RNN', 'R_3D_CNN']
    ...
    for data_file in data_file_list:
        ...
        for loop in range(2):
            ...
            oa, aa, kappa, ac_list, loss, f1, pred, labels, matrix_pro, mse = model_i.voting_test(test_data_loader, models_weights, voting_models)
        ...
        oa, aa, kappa, ac_list, loss, f1, pred, labels, matrix_pro, mse = model_i.metrics_pro(pred_arr, labels)
    ...
    print(df)
    df.to_csv(os.path.join(args.result, data_type + '_' + str(file_no) + '_softvoting_kfold_test_scores_reg.csv'), index=False)
    pickle.dump(matrix_pro_list, open(
        os.path.join(args.result, data_type + '_' + str(file_no) + '_softvoting_kfold_matrix_probabilities_reg.pkl'), 'wb'))

    print('softvoting_test_score_list:')
    df = pd.DataFrame(data=voting_test_score_list,
                      columns=['algorithm', 'data_file', 'oa', 'aa', 'kappa', 'acc_list', 'loss', 'f1'])
    print(df)
    df.to_csv(os.path.join(args.result, data_type + '_' + str(file_no) + '_softvoting_test_scores_reg.csv'), index=False)
    pickle.dump(voting_matrix_pro_list, open(
        os.path.join(args.result, data_type + '_' + str(file_no) + '_softvoting_matrix_probabilities_reg.pkl'), 'wb'))
