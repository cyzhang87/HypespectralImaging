...
class Data():
    def __init__(self, args):
        ...
        self.data_dict = sio.loadmat(str(pathlib.Path(self.data_path, self.data_file, self.data_name + '.mat')))
    ...
    def read_data(self):
        ...
        for i in range(1, class_num + 1):
            xx = np.array(data_pos[i])[:, 0]
            x0 = (np.min(xx) + np.max(xx)) // 2
            yy = np.array(data_pos[i])[:, 1]
            y0 = (np.min(yy) + np.max(yy)) // 2
            center_pos[i].append([x0, y0])
        #将每个标注数据进行4等分
        splits_pos = {i: [] for i in range(1, class_num + 1)}
        for i in range(1, class_num + 1):
            pos_i = np.array(data_pos[i])
            """
            splits_pos[i].append(list(pos_i[pos_i[:, 0] < (center_pos[i][0][0] - 1)]))  #第1, 2部分属于training set
            splits_pos[i].append(list(pos_i[(pos_i[:, 0] > (center_pos[i][0][0] + 1)) & (
                        pos_i[:, 1] < (center_pos[i][0][1] - 1))]))  # 第3部分属于validation set
            splits_pos[i].append(list(pos_i[(pos_i[:, 0] > (center_pos[i][0][0] + 1)) & (
                        pos_i[:, 1] > (center_pos[i][0][1] + 1))]))  # 第4部分属于validation set
            """
            splits_pos[i].append(list(pos_i[pos_i[:, 1] < (center_pos[i][0][1] - 1)]))  # 第1, 2部分属于training set
            splits_pos[i].append(list(pos_i[(pos_i[:, 0] < (center_pos[i][0][0] - 1)) & (
                    pos_i[:, 1] > (center_pos[i][0][1] + 1))]))  # 第3部分属于validation set
            splits_pos[i].append(list(pos_i[(pos_i[:, 0] > (center_pos[i][0][0] + 1)) & (
                    pos_i[:, 1] > (center_pos[i][0][1] + 1))]))  # 第4部分属于validation set

        data_gt_tmp = data_gt.copy()
        for k, v in splits_pos.items():
            for i in v[0]:
                data_gt_tmp[i[0], i[1]] = 10

            for i in v[1]:
                data_gt_tmp[i[0], i[1]] = 20

            for i in v[2]:
                data_gt_tmp[i[0], i[1]] = 30
        import matplotlib.pyplot as plt
        fig = plt.figure()
        ax = fig.add_subplot(111)
        im = ax.imshow(data_gt_tmp)
        plt.colorbar(im)
        plt.show()
        for k, v in splits_pos.items():
            if self.train_num > 0 and self.train_num < 1:
                train_num = self.train_num * len(v[0])
            elif len(v[0])<self.train_num:
                train_num = len(v[0])
            else:
                train_num = self.train_num
            train_pos[k] = random.sample(v[0], int(train_num))
            val_pos[k] = v[1]
            test_pos[k] = v[2]
        ...
        for k,v in self.val_pos.items():
            for t in v:
                val_pos_all.append([k,t])
        ...
        val_t = 0
        test_t = 0
        for (k1,v1), (k2,v2), (k3,v3) in zip(self.train_pos.items(),self.val_pos.items(), self.test_pos.items()):
            print('traindata-ID %s: %s; valdata-ID %s: %s; testdata-ID %s: %s'%(k1,len(v1),k2,len(v2),k3,len(v3)))
            train_t += len(v1)
            val_t += len(v2)
            test_t += len(v3)
        print('total train %s, total val %s, total test %s'%(train_t, val_t, test_t))
        # for k,v in self.test_pos.items():
        ...
        # val data
        val_data_name = os.path.join(self.tfrecords, 'val_data.tfrecords')
        writer = tf.python_io.TFRecordWriter(val_data_name)
        for i in val_pos_all:
            [r,c] = i[1]
            pixel_t = self.neighbor_add(r,c,w_size=self.cube_size).astype(np.float32).tostring()
            label_t = np.array(np.array(i[0] - 1).astype(np.int64))
            example = tf.train.Example(features=(tf.train.Features(
    ...
    def data_parse(self,filename,type='train'):
        ...
        if type == 'train':
            ...
            train_data_dict = {}
            train_data_dict['data'] = torch.tensor(train_data, dtype=torch.float32)
            train_data_dict['label'] = torch.tensor(train_label, dtype=torch.long)

            return train_loader, train_data_dict
            """
            X_train = torch.tensor(train_data, dtype=torch.float32)
            y_train = torch.tensor(train_label, dtype=torch.long)
            torch_dataset = TorchData.TensorDataset(X_train, y_train)
